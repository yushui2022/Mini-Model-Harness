async (page) => {
  await page.evaluate(() => {
    ...
    window.fetch = (input, init) => {
      ...
      const events = [
        { delay: 0, payload: { type: "start", total: 2, startedAt: new Date().toISOString(), profile: { provider: "mock", model: "mock-3b" } } },
        {
          delay: 300,
          payload: {
            type: "case",
            ...
            total: 2,
            case: passCase,
            summary: { total: 2, completed: 1, passed: 1, failed: 0, passRate: 1, avgScore: 1, latencyMs: 120, failureTagCounts: {} }
          }
        },
        {
          delay: 1200,
          payload: {
            ...
            total: 2,
            case: failCase,
            summary: { total: 2, completed: 2, passed: 1, failed: 1, passRate: 0.5, avgScore: 0.5, latencyMs: 300, failureTagCounts: { JSON_PARSE_ERROR: 1 } }
          }
        },
        {
          delay: 1500,
  ...
  const readState = async () => page.evaluate(() => {
    ...
    return {
      ...
      hasFirstCase: Boolean(output?.textContent.includes("mock_live_first")),
      hasSecondCase: Boolean(output?.textContent.includes("mock_live_second")),
      hasParsedOutput: Boolean(output?.textContent.includes("解析结果")),
      hasDiagnostics: Boolean(output?.textContent.includes("诊断"))
    };
  });
